...
figure(1)
BLOCK_LENGTH = 12.872;%单位是A
plot(t_nep,jp_nep/BLOCK_LENGTH,'-','linewidth',3)
xlabel('Time (ns)')
...
figure(2)
plot(t_nep,jp_nep./etol_nep,'-','linewidth',3)
xlim([0 2])
ylim([10 16])
xticks([0 0.5 1.0 1.5 2.0])
...
jp=compute(Index_begin:end,29:end-2);%前13列为输出温度
jpy = jp(:, 74); % in units of eV^{3/2} amu^{-1/2}，注意1和12组为热浴组
% jp的1-13为jpx，skip；
% 14为一项不知名列，15为热浴（即是组1）
% 26列为组12，也是热浴

% python语句中，是15：25，因为从0开始索引，并且不包含25
BLOCK_LENGTH = 12.87;%单位是A
%jpy = jpy / BLOCK_LENGTH * 98207.9; % #98207.9将ev^{3/2} amu^{-1/2} A^{-1}转换为eV/ns
jpy = jpy  * 98207.9; % #98207.9将ev^{3/2} amu^{-1/2} A^{-1}转换为eV/ns
jpy_ave = mean(jpy, 2);
