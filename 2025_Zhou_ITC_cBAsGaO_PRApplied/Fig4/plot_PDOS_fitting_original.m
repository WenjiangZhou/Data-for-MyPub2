...
hold on
plot(x,y(:,2),'-','linewidth',2,'Color',colorred)
hold on
plot(x,y(:,3),'-','linewidth',1,'Color',orange)
hold on
plot(x,y(:,4),'linewidth',2,'Color',orange)
xlim([0,10])
% ylim([-0.01,0.02]) 
...
figure(2)
plot(x,y(:,5),'linewidth',1,'Color',colorred)
hold on
plot(x,y(:,6),'-','linewidth',2,'Color',colorred)
hold on
plot(x,y(:,7),'-','linewidth',1,'Color',orange)
hold on
plot(x,y(:,8),'linewidth',2,'Color',orange)
xlim([0,10])
xlim([0,10])
% ylim([-0.01,0.02]) 
xlabel("Frequency (THz)",'Fontname', 'Times New Roman', 'Fontsize', Fontsize)
...
data=load('.\Energy_dos_fitting_original.txt');
x=data(:,1);
y=data(:,2:end);
end
